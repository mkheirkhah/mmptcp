...
//        }
  ...
  if (ptrDSN == 0)
    {
      NS_LOG_INFO ("Retransmit -> no Unacked data !! mapDSN size is "<< sFlow->mapDSN.size() << " max Ack seq nï¿½ï¿½ "<< sFlow->highestAck << " (" << (int)sFlowIdx<< ")");
      NS_ASSERT(3!=3);
      return;
    }
...
MMpTcpSocketBase::DoGenerateOutPutFile()
{
  ...
  *os << "[:" << m_node->GetId() << ":][+" << flowId << "+][=" << flowType << "=][#" << goodput / 1000000 << "#][*"
      ...
      << subflows[(uint32_t) currentSublow]->lastMeasuredRtt.GetSeconds() << "/][~" << subflows[0]->m_retxThresh << "~][¤"
      << subflows[0]->rtt->GetMinRto().GetMilliSeconds() << "¤][{" << subflows[0]->m_limitedTx << "}][-" << m_isThinStream
      << "-][|" << m_isRTObackoff << "|]" << endl;
